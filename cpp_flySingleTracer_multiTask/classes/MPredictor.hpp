...
public:
	...
	vector<Rect> predictOneLine(Mat& imgGray, Size modelWin = Size(10, 100), int wRoi = 16) {
		...
		for (int i = 0; i < hM*wM; i++) {
			pMap[i] = (uchar)max(0.0f, 125 + 125 * pY[i * 2 + 1] - 125 * pY[i * 2]);
		}
	...
	vector<Rect> predictMultipleLine(Mat& imgGray, Size modelWin = Size(10, 100), int wRoi = 16) {
		// setup environment of roi batch setting
		string wConfig = "ROI batch configuration";
		namedWindow(wConfig);
		createTrackbar("roi width", wConfig, &wRoi, 20);  // create a trackbar for adjustment of rois width
		Mat imgLabel; cvtColor(imgGray, imgLabel, CV_GRAY2BGR);
		MRectRoi* roi = NULL;
		vector<Rect> tubeRois;
		vector<Rect> currBatch;
		int key = -1;
		do {  // loop: ask to draw a large rectangle, and use predictOneLine to automatically detect tubes
			roi = new MRectRoi();
			key = roi->setRoi(wConfig.c_str(), imgLabel);  // set current large roi and get key value
			if (roi->getRect().width * roi->getRect().height != 0) {  // only save valid roi
				// constrain wRoi range
				if (wRoi < 10) {
					wRoi = 10;
					cout << "wRoi should be within [10, 20].\n";
				}
				// adjust current batch of tube rois
				currBatch = this->predictOneLine(imgGray(roi->getRect()), modelWin, wRoi);  // get current rois
				for (size_t i = 0; i < currBatch.size(); i++) {  // append current batch of tube rois
					tubeRois.push_back(Rect(
						roi->getRect().x + currBatch[i].x,  // need to compensate the subimage offset
						roi->getRect().y + currBatch[i].y,
						currBatch[i].width, 
						currBatch[i].height
						));
					rectangle(imgLabel, tubeRois[tubeRois.size()-1], Scalar(0, 0, 255));
				}
				// show feedback
				imshow(wConfig, imgLabel);
			}
		} while (key != 27);  // stop: when enter <esc>; continue: when enter <space>
...
private:  // helper for parse() interface
	...
	void getData(ifstream& ifs, Mat& m) {
		...
		while (1) {
			...
			vector<string> dat = this->split(line);
