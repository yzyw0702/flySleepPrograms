...
def saveData(fImg, lDeathTime, lDeathPhase):
	hDt = None
	if not os.path.exists('deathtime.txt'):
		hDt = open('deathtime.txt', 'w')
	else:
		hDt = open('deathtime.txt', 'a')
	...
	hDt.close()
	hDp = None
	if not os.path.exists('config_flys-corrected.txt'):
		hDp = open('config_flys-corrected.txt', 'w')
		hDp.write('line\tindi\tfrag\n')
	else:
		hDp = open('config_flys-corrected.txt', 'a')
...
def runtask(rootpath):
	...
	for i, fImg in enumerate(lfRaster):
		...
		saveData(fImg, lDeathTime, lDeathPhase)
